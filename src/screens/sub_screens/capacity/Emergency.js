...
export default function Emergency() {
    ...
    const [value, setValue] = useState(const_data.dataOfHospital.data.beds.toString());
    const [patientsVal, setPatientsVal] = useState(const_data.dataOfHospital.data.beds.toString())
    const {updateHospital} = useAuth();
    
    const SaveChanges =()=>{
        setUpdate(false)
        setPatientsVal(value)
        updateHospital({beds: value})
    }
                        ...
                        <Col className='d-flex align-items-center flex-column' style={{flex:1, marginTop: 80}} >
                            <Card style={{borderRadius: 10, width:'fit-content', height:'fit-content'}}>
                                <Card.Body className='d-flex calendar'>
                                    <img alt='calendar' src='/Images/capacity/calendar.png' />
                                    <span>{data.date}</span>
                                </Card.Body>
                            </Card>
                            ...
                            <div style={{justifyContent: 'center',alignItems:'center', display:'flex', flexDirection: 'column'}}>
                                <Card  className='shadow p-3 mb-5 bg-white rounded'style={{width:'fit-content', height:'fit-content'}}>
                                    <Card.Body style={{justifyContent:'center', display:'flex'}}>
                                        <img alt='hospital_bed' src='/Images/capacity/hospital-bed.png' className='capacity_img'/>
                                        <div className='dashboard_card_text'style={{alignSelf:'center'}}>
                                            <h1>ER beds</h1>
                                            <h1 style={{textAlign:'center', color:'#00cf87', fontWeight: 'bold'}}>{patientsVal}</h1>
                                        </div>
                                    </Card.Body>
                                </Card>
                                <div style={{height:50}}>
                                    { !update ? <h1 className='capacity_text'>Current number of occupied beds</h1> : null}
                                    { update ? <div className='align-items-center d-flex justify-content-center'>
                                        <img alt='plus' src='/Images/capacity/plus.png' 
                                        ...
                                        />
                                        <img alt='minus' src='/Images/capacity/minus.png' 
                                        style={{alignSelf:'center',
                                                height:30,
                                                width:30,
                                                marginTop: 20,
                                    ...
                                    {update?<h1 style={{fontSize: 15, marginTop: 40}}>
                                        Enter the number of increased or decreased beds:
                                    </h1>: null}
                                </div>
